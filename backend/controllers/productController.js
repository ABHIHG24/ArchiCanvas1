...
exports.createProduct = async (req, res) => {
  try {
    const { title, description, price, isBiddable, biddingDays, tags } =
      req.body;
...
exports.notifyArtistForPurchase = async (req, res, next) => {
  try {
    ...
    if (!product.user) {
      return next(
        new ErrorHandler(
          "The artist for this product could not be found.",
          404,
        ),
      );
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Purchase inquiry sent to the artist!" });
  } catch (error) {
    next(error);
  }
};
